...
MiniFiles.config = {
  ...
  options = {
    ...
    fs_actions_confirm = true,
    close_on_file_opened = false,
    open_on_current_dir = false,
  },
...
MiniFiles.open = function(path, use_latest, opts)
	...
	if MiniFiles.config.options.open_on_current_dir then
		default_path = vim.api.nvim_buf_get_name(0)
	end
	path = H.fs_full_path(path or default_path)
...
H.explorer_refresh = function(explorer, opts)
	...
	if H.do_trigger_events == nil then
		H.do_trigger_events = true
	end
...
H.explorer_sync_cursor_and_branch = function(explorer, depth)
	...
	if show_preview and is_dir and is_cur_buf then
...
H.explorer_go_in_range = function(explorer, buf_id, from_line, to_line)
	...
	if dir_path ~= nil then
		...
		if H.is_valid_win(win_id) then
			vim.api.nvim_win_set_cursor(win_id, { dir_line, 0 })
		end
	end
...
H.explorer_confirm_modified = function(explorer, action_name)
	...
	for _, dir_view in pairs(explorer.dir_views) do
		if H.is_modified_buffer(dir_view.buf_id) then
			has_modified = true
		end
	end
...
H.dir_view_ensure_proper = function(dir_view, dir_path, opts)
	...
	if type(dir_view.cursor) == "string" then
		dir_view = H.dir_view_decode_cursor(dir_view)
	end
...
H.fs_actions_confirm = function(fs_actions)
	if not MiniFiles.config.options.fs_actions_confirm then
		return true
	end
...
H.trigger_event = function(event_name, data)
	if not H.do_trigger_events then
		return
	end
