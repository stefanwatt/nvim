...
vim.api.nvim_create_autocmd("FileType", {
	...
	callback = function(event)
		...
		local qf_opts = trouble and trouble.is_open() and vim.tbl_extend("force", opts, { buffer = event.buf }) or opts

		vim.keymap.set({ "n", "i" }, "<C-p>", "<cmd>cprev<CR>", qf_opts)
		vim.keymap.set({ "n", "i" }, "<C-n>", "<cmd>cnext<CR>", qf_opts)

		vim.api.nvim_create_autocmd("BufWinLeave", {
			buffer = event.buf,
			once = true,
			callback = function()
				vim.keymap.del({ "n", "i" }, "<C-p>", { buffer = event.buf })
				vim.keymap.del({ "n", "i" }, "<C-n>", { buffer = event.buf })
			end,
		})
	end,
})
...
vim.keymap.set("n", "<leader>gg", function()
	utils.i3_exec("/usr/bin/wezterm start --class foo --always-new-process --cwd " .. vim.uv.cwd() .. " -- lazygit")
end, opts)
