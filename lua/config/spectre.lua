local Renderer = require("config.nui-components")
...
M.toggle = function()
	...
	local search = function()
		...
		spectre_state.finder_instance =
			search_engine:new(spectre_state_utils.get_search_engine_config(), search_handler())
	...
	local body = h.rows(h.columns(
		...
		h.rows(
			...
			h.text_input({
				...
				on_change = fn.debounce(function(value)
					local search_paths = fn.imap(vim.split(value, ","), function(path)
						return fn.trim(path)
					end)
				...
				prepare_node = function(node, line, component)
					...
					if has_children then
						...
						line:append(
							...
							component:make_highlight_group_name("SpectreIcon")
						)
					...
					local search_highlight_group = component:make_highlight_group_name(
