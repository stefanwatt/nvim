...
function Renderer:get_component_by_id(id)
	return fn.ifind(self.flatten_tree, function(component)
...
function Renderer:__get_layout_box()
	local components = fn.ireduce(self.tree, function(acc, component)
...
function Renderer:__listen_on_state_changes()
	if self.layout then
		fn.ieach(self.flatten_tree, function(component)
...
function Renderer:__determine_redraw_need()
	local trigger_values = fn.imap(self.flatten_tree, function(component)
...
function Renderer:__determine_focusable_components()
	self.focusable_components = fn.ifilter(self.flatten_tree, function(component)
...
function Renderer:__set_components_tree(root)
	local function rec(content, parent)
		return fn.ireduce(content, function(acc, element, index)
...
function Renderer:focus()
	if self.layout then
		...
		if last_focused_component then
			...
			local first_focusable_component = fn.ifind(self.flatten_tree, function(component)
