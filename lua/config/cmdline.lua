...
vim.api.nvim_set_hl(0, "CmdRegular", {
    fg = colors.blue,
})

vim.api.nvim_set_hl(0, "CmdRegularSolid", {
    fg = colors.base,
    bg = colors.blue,
})

vim.api.nvim_set_hl(0, "CmdLua", {
    fg = colors.mauve,
})

vim.api.nvim_set_hl(0, "CmdLuaSolid", {
    fg = colors.base,
    bg = colors.mauve,
})
	...
	default = {
		winopts = {
			title = {
				{ "", "CmdRegular" },
				{ "  ", "CmdRegularSolid" },
				{ "v" .. vim.version().major .. "." .. vim.version().minor .. " ", "CmdRegularSolid" },
				{ "", "CmdRegular" },
			},
			title_pos = "right",
		},
		icon = { { " 󰣖 ", "CmdRegular" } },
		winhl = "FloatBorder:Directory,Normal:Normal",
		ft = "vim",
	},
	configs = {
		{
			firstc = ":",
			match = "^s/",
			icon = { { "  ", "LspDiagnosticsWarning" } },

			winopts = {
				-- Position the window in the top right corner
				anchor = "NE",
				-- Set a fixed height of 3 rows
				height = 3,
		...
		},
		{
			...
			text = function(inp)
				...
				if #parts >= 3 then
					flags = table.concat(parts, "", 3)
				end
				-- Modify the format to fit in 3 rows
				local result = string.format("P: %s\nR: %s\nF: %s", pattern, replacement, flags)
				return result
			end,
		},
		{
		...
		},
		{
			...
			winopts = {
				title = {
					{ "", "CmdLua" },
					{ "  " .. _VERSION .. " ", "CmdLuaSolid" },
					{ "", "CmdLua" },
				},
				title_pos = "right",
			},

			winhl = "FloatBorder:csvCol7,Normal:Normal",
			icon = { { "  ", "CmdLua" } },
			ft = "lua",
		...
		},
		{
			...
			icon = { { "  ", "csvCol1" } },
			winhl = "FloatBorder:csvCol1,Normal:Normal",
		},
		{
			firstc = "/",
			winopts = {
			...
			icon = { { "  ", "LspDiagnosticsWarning" } },
			winhl = "FloatBorder:LspDiagnosticsWarning,Normal:Normal",
		},
		{
			firstc = "=",
			winopts = {
			...
			icon = { { " 󰇼 ", "diffAdded" } },
			winhl = "FloatBorder:diffAdded,Normal:Normal",
		},
	},
...
cmd.update_state = function(state)
	...
	for _, conf in ipairs(cmd.conf.configs) do
		if (not conf.firstc or conf.firstc == cmd.state.firstc) and (not conf.match or txt:match(conf.match)) then
